...
2. 點擊「空白」建立新試算表
3. 將試算表命名為 "Atomic Task Matrix"（或任意名稱）

**步驟 2：建立 Tasks 分頁**
1. 將第一個分頁重新命名為 `Tasks`
2. 在第一行（表頭）依序輸入以下欄位名稱：
   ```
   A1: id
   B1: title
   C1: status
   D1: parent_task_id
   E1: parent_task_title
   F1: created_at
   G1: updated_at
   H1: completed_at
   ```

**步驟 3：建立 Analytics 分頁**
1. 點擊左下角 ＋ 號新增分頁
2. 將新分頁命名為 `Analytics`
3. 在第一行（表頭）依序輸入以下欄位名稱：
   ```
   A1: week_start
   B1: week_end
   C1: total_created
...
1. 查看瀏覽器網址列，格式如下：
   ...
   ```
2. 複製 `/d/` 和 `/edit` 之間的長字串（這就是 Spreadsheet ID）
3. 將 ID 填入 `gas/backend.gs` 的 `CONFIG.SPREADSHEET_ID`
